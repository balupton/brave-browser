            ...
            steps {
                script {
                    if (SKIP) {
                        ...
                        checkout([$class: "GitSCM", branches: [[name: "${BRANCH}"]], extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'brave-browser']], userRemoteConfigs: [[url: "https://github.com/brave/brave-browser.git"]]])
                        checkout([$class: "GitSCM", branches: [[name: "${BRANCH}"]], extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'brave-core']], userRemoteConfigs: [[url: "https://github.com/brave/brave-core.git"]]])
                    }
                    else
                    {
                        checkout([$class: "GitSCM", branches: [[name: "${BRANCH}"]], extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'brave-browser']], userRemoteConfigs: [[url: "https://github.com/brave/brave-browser.git"]]])
                        BC_BRANCH = sh(script: 'jq -r \'.config.projects["brave-core"].branch\' brave-browser/package.json', returnStdout: true)
                        checkout([$class: "GitSCM", branches: [[name: "${BC_BRANCH}"]], extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'brave-core']], userRemoteConfigs: [[url: "https://github.com/brave/brave-core.git"]]])
                    }
                    def bb_version = readJSON(file: 'brave-browser/package.json').version
                    def bc_version = readJSON(file: 'brave-core/package.json').version
